...
class DocumentModel extends Model{
    ...
    protected function _initialize()
    {
        ...
        $this->_validate =array(
            array('name', '/^[a-zA-Z]\w{0,39}$/', L('L_MODEL_DOC_1'), self::VALUE_VALIDATE, 'regex', self::MODEL_BOTH),
            array('name', 'checkName', L('L_MODEL_DOC_2'), self::VALUE_VALIDATE, 'callback', self::MODEL_BOTH),
            array('title', 'require', L('L_MODEL_DOC_3'), self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
            array('title', '1,80', L('L_MODEL_DOC_4'), self::MUST_VALIDATE, 'length', self::MODEL_BOTH),
            array('level', '/^[\d]+$/', L('L_MODEL_DOC_5'), self::VALUE_VALIDATE, 'regex', self::MODEL_BOTH),
            //TODO: 外链编辑验证
            //array('link_id', 'url', '外链格式不正确', self::VALUE_VALIDATE, 'regex', self::MODEL_BOTH),
            array('description', '1,140', L('L_MODEL_DOC_6'), self::VALUE_VALIDATE, 'length', self::MODEL_BOTH),
            array('category_id', 'require', L('L_MODEL_DOC_7'), self::MUST_VALIDATE , 'regex', self::MODEL_INSERT),
            array('category_id', 'require', L('L_MODEL_DOC_8'), self::EXISTS_VALIDATE , 'regex', self::MODEL_UPDATE),
            array('category_id', 'checkCategory', L('L_MODEL_DOC_9'), self::EXISTS_VALIDATE , 'callback', self::MODEL_UPDATE),
            array('model_id,category_id', 'checkModel', L('L_MODEL_DOC_10'), self::MUST_VALIDATE , 'callback', self::MODEL_INSERT),
            array('deadline', '/^\d{4,4}-\d{1,2}-\d{1,2}(\s\d{1,2}:\d{1,2}(:\d{1,2})?)?$/', L('L_MODEL_DOC_11'), self::VALUE_VALIDATE  , 'regex', self::MODEL_BOTH),
            array('create_time', '/^\d{4,4}-\d{1,2}-\d{1,2}(\s\d{1,2}:\d{1,2}(:\d{1,2})?)?$/', L('L_MODEL_DOC_12'), self::VALUE_VALIDATE  , 'regex', self::MODEL_BOTH),
        );
    }
    ...
    public function update($data = null){
        ...
        if(empty($data['id'])){ //新增数据
            $id = $this->add(); //添加基础内容
            if(!$id){
                $this->error = L('L_MODEL_DOC_14');
                return false;
            }
        } else { //更新数据
            $status = $this->save(); //更新基础内容
            if(false === $status){
                $this->error = L('L_MODEL_DOC_15');
                return false;
            }
        }
    ...
    public function checkDocumentType($type = null, $pid = null){
		...
		if(empty($type)){
			return array('status'=>0, 'info'=>L('L_MODEL_DOC_20'));
		}
		if(empty($pid)){
			return $res;
		}
		...
		if($ptype == 2){
			if($type != 3){
				return array('status'=>0, 'info'=>L('L_MODEL_DOC_21'));
			}else{
				return $res;
			}
		}
		//父文档为段落时
		if($ptype == 3){
			return array('status'=>0, 'info'=>L('L_MODEL_DOC_22'));
		}
		return array('status'=>0, 'info'=>L('L_MODEL_DOC_23'));
    }

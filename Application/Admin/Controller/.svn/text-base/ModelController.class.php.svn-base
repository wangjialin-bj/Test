...
class ModelController extends AdminController {
    ...
    public function index(){
        ...
        $this->meta_title = L('L_ACTION_MODEL_1');
        $this->display();
    }
    ...
    public function add(){
        ...
        $this->meta_title = L('L_ACTION_MODEL_2');
        $this->display();
    }
    ...
    public function edit(){
        ...
        $this->meta_title = L('L_ACTION_MODEL_4');
        $this->display();
    }
    ...
    public function del(){
        $ids = I('get.ids');
        empty($ids) && $this->error(L('L_ACTION_MODEL_5'));
        $ids = explode(',', $ids);
        ...
        if(!$res){
            ...
            $this->success(L('L_ACTION_MODEL_6'));
        }
    }
    ...
    public function generate(){
        if(!IS_POST){
            ...
            $this->meta_title = L('L_ACTION_MODEL_7');
            $this->display();
        }else{
            $table = I('post.table');
            empty($table) && $this->error(L('L_ACTION_MODEL_8'));
            $res = D('Model')->generate($table);
            if($res){
                $this->success(L('L_ACTION_MODEL_9'), U('index'));
            }else{
                $this->error(D('Model')->getError());
            }
        }

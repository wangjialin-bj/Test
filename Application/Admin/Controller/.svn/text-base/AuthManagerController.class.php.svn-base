...
class AuthManagerController extends AdminController{
    ...
    public function index(){
        ...
        $this->meta_title = L('L_ACTION_META_9');
        $this->display();
    }
    ...
    public function createGroup(){
        ...
        $this->meta_title = L('L_ACTION_META_10');
    ...
    public function editGroup(){
        ...
        $this->meta_title = L('L_ACTION_META_11');
        $this->display();
    }
    ...
    public function access(){
        ...
        $this->meta_title = L('L_ACTION_META_12');
    ...
    public function writeGroup(){
        ...
        if ( $data ) {
            ...
            if($r===false){
                $this->error(L('L_ACTION_ERROR').$AuthGroup->getError());
            } else{
                $this->success(L('L_ACTION_SUCCESS'),U('index'));
            }
        }else{
            $this->error(L('L_ACTION_ERROR').$AuthGroup->getError());
        }
    ...
    public function changeStatus($method=null){
        ...
        switch ( strtolower($method) ){
            ...
            case 'deletegroup':
                ...
                break;
            default:
                $this->error($method.L('L_ACTION_AUTH_3'));
        }
    }
    ...
    public function user($group_id){
        ...
        $this->assign('auth_group', $auth_group);
        $this->assign('this_group', $auth_group[(int)$_GET['group_id']]);
        $this->meta_title = L('L_ACTION_META_13');
        $this->display();
    }
    ...
    public function category(){
        ...
        $this->assign('auth_group',     $auth_group);
        $this->assign('this_group',     $auth_group[(int)$_GET['group_id']]);
        $this->meta_title = L('L_ACTION_META_14');
        $this->display();
    }
    ...
    public function addToGroup(){
        ...
        $gid = I('group_id');
        if( empty($uid) ){
            $this->error(L('L_ACTION_AUTH_1'));
        }
        $AuthGroup = D('AuthGroup');
        if(is_numeric($uid)){
            if ( is_administrator($uid) ) {
                $this->error(L('L_ACTION_AUTH_5'));
            }
            if( !M('Member')->where(array('uid'=>$uid))->find() ){
                $this->error(L('L_ACTION_AUTH_6'));
            }
        }
        ...
        if ( $AuthGroup->addToGroup($uid,$gid) ){
            $this->success(L('L_ACTION_SUCCESS'));
        }else{
    ...
    public function removeFromGroup(){
        ...
        if( !$AuthGroup->find($gid)){
            $this->error(L('L_ACTION_AUTH_2'));
        }
        if ( $AuthGroup->removeFromGroup($uid,$gid) ){
            $this->success(L('L_ACTION_SUCCESS'));
        }else{
            $this->error(L('L_ACTION_ERROR'));
        }
    }

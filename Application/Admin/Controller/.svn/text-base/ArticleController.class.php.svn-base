...
class ArticleController extends AdminController {
    ...
    protected function indexOfReply($cate_id) {
        ...
        $this->meta_title = L('L_ACTION_META_5_0');
    ...
    protected function indexOfArticle($cate_id){
        ...
        $this->meta_title = L('L_ACTION_META_5');
        return 'index';
    }
    ...
    public function add(){
        ...
        empty($cate_id) && $this->error(L('L_ACTION_DOC_2'));
        empty($model_id) && $this->error(L('L_ACTION_DOC_3'));

        //检查该分类是否允许发布
        $allow_publish = D('Document')->checkCategory($cate_id);
        !$allow_publish && $this->error(L('L_ACTION_DOC_4'));
        ...
        $this->meta_title = L('L_ACTION_DOC_5').$model['title'];
        $this->display();
    }
    ...
    public function edit(){
        ...
        $this->meta_title   =   L('L_ACTION_META_5_1');
        $this->display();
    }
    ...
    public function batchOperate(){
        ...
        empty($cate_id) && $this->error(L('L_ACTION_DOC_7'));

        //检查该分类是否允许发布
        $allow_publish = D('Document')->checkCategory($cate_id);
        !$allow_publish && $this->error(L('L_ACTION_DOC_8'));
        ...
        if(IS_POST){
            ...
            if($res){
                $this->success(L('L_ACTION_DOC_9'), U('index?pid='.$pid.'&cate_id='.$cate_id));
            }else{
                if(isset($res)){
                    $this->error(D('Document')->getError());
                }else{
                    $this->error(L('L_ACTION_DOC_10'));
                }
            }
        }
        ...
        $this->meta_title       =   L('L_ACTION_META_5_2');
    ...
    public function examine(){
        ...
        $this->assign('list', $list);
        $this->meta_title       =   L('L_ACTION_META_5_3');
        $this->display();
    }
    ...
    public function recycle(){
        ...
        $this->assign('list', $list);
        $this->meta_title       =   L('L_ACTION_META_5_4');
        $this->display();
    }
    ...
    public function autoSave(){
        ...
        if($res !== false){
            $return['data']     =   $res;
            $return['info']     =   L('L_ACTION_DOC_11');
            $return['status']   =   1;
            $this->ajaxReturn($return);
        }else{
            $this->error(L('L_ACTION_DOC_12').D('Document')->getError());
        }
    }
    ...
    public function draftBox(){
        ...
        $this->assign('list', $list);
        $this->meta_title = L('L_ACTION_META_5_5');
        $this->display();
    }
    ...
    public function mydocument($status = null, $title = null){
        ...
        $this->assign('status', $status);
        $this->assign('list', $list);
        $this->meta_title = L('L_ACTION_META_5_6');
        $this->display();
    }
    ...
    public function clear(){
        ...
        if($res !== false){
            $this->success(L('L_ACTION_DOC_14'));
        }else{
            $this->error(L('L_ACTION_DOC_15'));
        }
    }
    ...
    public function move() {
        if(empty($_POST['ids'])) {
            $this->error(L('L_ACTION_DOC_16'));
        }
        session('moveArticle', $_POST['ids']);
        session('copyArticle', null);
        $this->success(L('L_ACTION_DOC_17'));
    }
    ...
    public function copy() {
        if(empty($_POST['ids'])) {
            $this->error(L('L_ACTION_DOC_18'));
        }
        session('copyArticle', $_POST['ids']);
        session('moveArticle', null);
        $this->success(L('L_ACTION_DOC_19'));
    }
    ...
    public function paste() {
        ...
        if(empty($moveList) && empty($copyList)) {
            $this->error(L('L_ACTION_DOC_20'));
        }
        if(!isset($_POST['cate_id'])) {
            $this->error(L('L_ACTION_DOC_21'));
        }
        ...
        if(!empty($moveList)) {// 移动	TODO:检查name重复
            ...
            session('moveArticle', null);
            if(false !== $res){
                $this->success(L('L_ACTION_DOC_22'));
            }else{
                $this->error(L('L_ACTION_DOC_23'));
            }
            ...
            session('copyArticle', null);
            if($res){
                $this->success(L('L_ACTION_DOC_24'));
            }else{
                $this->error(L('L_ACTION_DOC_25'));
            }
        }
    }
    ...
    protected function checkPaste($list, $cate_id, $pid){
        ...
        foreach ($list as $key=>$value){
            ...
            if($value == $pid){
                $return['status'] = 0;
                $return['info'] = L('L_ACTION_DOC_26').$value.L('L_ACTION_DOC_27');
                return $return;
            }
            // 移动文档的所属文档模型
            ...
            if(!in_array($modelType,explode(',',$modelList))) {
                $return['status'] = 0;
                $return['info'] = L('L_ACTION_DOC_28').$value.L('L_ACTION_DOC_29');
                return $return;
            }
        }
        ...
        foreach ($list as $key=>$value){
            ...
            if(!in_array($modelType,explode(',',$typeList))) {
                $return['status'] = 0;
                $return['info'] = L('L_ACTION_DOC_30').$value.L('L_ACTION_DOC_31');
                return $return;
            }
            $res = $Document->checkDocumentType($modelType, $pid);
            if(!$res['status']){
                $return['status'] = 0;
                $return['info'] = $res['info'].L('L_ACTION_DOC_32').$value;
                return $return;
            }
        }
    ...
    public function sort(){
        if(IS_GET){
            ...
            $this->assign('list', $list);
            $this->meta_title = L('L_ACTION_META_5_7');
            $this->display();
        }elseif (IS_POST){
            $ids = I('post.ids');
            ...
            if($res !== false){
                $this->success(L('L_ACTION_DOC_33'));
            }else{
                $this->eorror(L('L_ACTION_DOC_34'));
            }
        }else{

...
$(function(){
    ...
    $('.ajax-get').click(function(){
        ...
        if ( $(this).hasClass('confirm') ) {
            if(!confirm(print_var['confirm_to_perform'])){
                return false;
            }
        }
        if ( (target = $(this).attr('href')) || (target = $(this).attr('url')) ) {
            $.get(target).success(function(data){
                if (data.status==1) {
                    if (data.url) {
                        updateAlert(data.info + print_var['auto_jump'],'alert-success');
                    }else{
...
$(function(){
    ...
    //切换语言
    $('#changeLanguageStatus a').click(function(){
        $.post('index.php?s=/Admin/Admin/changeLanguage',
            {lang:$(this).attr('type') , time:new Date().getTime()},
            function(msg){
                if(msg){

...
class ModelModel extends Model{
    ...
    protected function _initialize()
    {
        ...
        $this->_validate = array(
            array('name', 'require', L('L_MODEL_MODEL_1'), self::MUST_VALIDATE , 'regex', self::MODEL_INSERT),
            array('name', '/^[a-zA-Z]\w{0,39}$/', L('L_MODEL_MODEL_2'), self::VALUE_VALIDATE, 'regex', self::MODEL_BOTH),
            array('name', '', L('L_MODEL_MODEL_3'), self::VALUE_VALIDATE, 'unique', self::MODEL_BOTH),
            array('title', 'require', L('L_MODEL_MODEL_4'), self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
            array('title', '1,30', L('L_MODEL_MODEL_5'), self::MUST_VALIDATE, 'length', self::MODEL_BOTH),
            array('list_grid', 'require', L('L_MODEL_MODEL_6'), self::MUST_VALIDATE , 'regex', self::MODEL_UPDATE),
        );
    }
    ...
    public function update(){
        ...
        if(empty($data['id'])){ //新增数据
            $id = $this->add(); //添加基础内容
            if(!$id){
                $this->error = L('L_MODEL_MODEL_7');
                return false;
            }
        } else { //更新数据
            $status = $this->save(); //更新基础内容
            if(false === $status){
                $this->error = L('L_MODEL_MODEL_8');
                return false;
            }
        }
		// 清除模型缓存数据

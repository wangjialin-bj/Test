...
class AddonsModel extends Model {
    ...
    public function getList($addon_dir = ''){
        ...
        foreach ($dirs as $value) {
            if(!isset($addons[$value])){
				...
				if(!class_exists($class)){ // 实例化插件失败忽略执行
					\Think\Log::record(L('L_MODEL_ADDONS_2').$value.L('L_MODEL_ADDONS_3'));
					continue;
				}
                $obj    =   new $class;
        ...
        int_to_string($addons, array('status'=>array(-1=>L('L_BTN_BROKEN'), 0=>L('L_BTN_DISABLE'), 1=>L('L_BTN_ENABLE'), null=>L('L_BTN_UNINSTALL'))));

...
class ThinkController extends AdminController {
    ...
    public function lists($model = null, $p = 0){
        $model || $this->error(L('L_ACTION_THINK_1'));
        $page = intval($p);
        $page = $page ? $page : 1; //默认显示第一页数据

        //获取模型信息
        $model = M('Model')->getByName($model);
        $model || $this->error(L('L_ACTION_THINK_2'));
        ...
        $this->meta_title = $model['title'].L('L_ACTION_THINK_3');
        $this->display($model['template_list']);
    }

    public function del($model = null, $ids=null){
        $model = M('Model')->find($model);
        $model || $this->error(L('L_ACTION_THINK_4'));

        $ids = array_unique((array)I('ids',0));

        if ( empty($ids) ) {
            $this->error(L('L_ACTION_THINK_5'));
        }
    ...
    public function edit($model = null, $id = 0){
        //获取模型信息
        $model = M('Model')->find($model);
        $model || $this->error(L('L_ACTION_THINK_6'));
        ...
        if(IS_POST){
            ...
            if($Model->create() && $Model->save()){
                $this->success(L('L_ACTION_THINK_7').$model['title'].L('L_ACTION_THINK_8'), U('lists?model='.$model['name']));
            } else {
            ...
            $data || $this->error(L('L_ACTION_THINK_9'));

            $this->assign('model', $model);
            $this->assign('fields', $fields);
            $this->assign('data', $data);
            $this->meta_title = L('L_ACTION_THINK_10').$model['title'];
    ...
    public function add($model = null){
        ...
        $model || $this->error(L('L_ACTION_THINK_11'));
        if(IS_POST){
            $Model  =   D(parse_name(get_table_name($model['id']),1));
            // 获取模型的字段信息 
            $Model  =   $this->checkAttr($Model,$model['id']);
            if($Model->create() && $Model->add()){
                $this->success(L('L_ACTION_THINK_12').$model['title'].L('L_ACTION_THINK_13'), U('lists?model='.$model['name']));
            } else {
                $this->error($Model->getError());
            }
        } else {
            ...
            $this->assign('model', $model);
            $this->assign('fields', $fields);
            $this->meta_title = L('L_ACTION_THINK_14').$model['title'];
    ...
    protected function checkAttr($Model,$model_id){
        ...
        foreach($fields as $key=>$attr){
            if($attr['is_must']){// 必填字段
                $validate[]  =  array($attr['name'],'require',$attr['title'].L('L_ACTION_THINK_15'));
            }
            // 自动验证规则

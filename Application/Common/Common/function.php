...
function send_mail($to = '', $subject = '', $body = '', $name = '', $attachment = null)
{
	...
	$reply_email = '';
	$reply_name = '';
	import('Vendor.PHPMailer.phpmailer');//从PHPMailer目录导入phpmailer.class.php类文件
...
function getUnReadMessageCount($uid = 0)
{
    if(!$uid)
    {
        return D('Message')->getUnReadCount(is_login());
    }
    return D('Message')->getUnReadCount($uid);
}
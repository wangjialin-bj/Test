...
class HooksModel extends Model {
    ...
    protected function _initialize()
    {
        ...
        $this->_validate = array(
            array('name','require',L('L_MODEL_HOOK_1')), //默认情况下用正则进行验证
            array('description','require',L('L_MODEL_HOOK_2')), //默认情况下用正则进行验证
        );
    }
    /**
     * 文件模型自动完成
    ...
    public function updateHooks($addons_name){
        ...
        if(!class_exists($addons_class)){
            $this->error = L('L_MODEL_HOOK_3')."{$addons_name}".L('L_MODEL_HOOK_4');
            return false;
        }

...
class AuthGroupModel extends Model {
    ...
    protected function _initialize()
    {
        ...
        $this->_validate =array(
            array('title','require', L('L_MODEL_AUTHGROUP_1'), Model::MUST_VALIDATE ,'regex',Model::MODEL_INSERT),
            //array('title','require', '必须设置用户组标题', Model::EXISTS_VALIDATE  ,'regex',Model::MODEL_INSERT),
            array('description','0,80', L('L_MODEL_AUTHGROUP_2'), Model::VALUE_VALIDATE , 'length'  ,Model::MODEL_BOTH ),
    ...
    public function addToGroup($uid,$gid){
        ...
        if ($Access->getDbError()) {
            if( count($uid_arr)==1 && count($gid)==1 ){
                //单个添加时定制错误提示
                $this->error = L('L_MODEL_AUTHGROUP_3');
            }
            return false;
        }else{
            return true;
     ...
     * @author 朱亚杰 <zhuyajie@topthink.net>
     */
    public function checkId($modelname,$mid,$msg = 'The following id does not exist:'){
        if(is_array($mid)){
            $count = count($mid);
            $ids   = implode(',',$mid);
        }else{
    ...
    public function checkGroupId($gid){
        return $this->checkId('AuthGroup',$gid, L('L_MODEL_AUTHGROUP_5'));
    }
    ...
    public function checkCategoryId($cid){
        return $this->checkId('Category',$cid, L('L_MODEL_AUTHGROUP_6'));
    }
